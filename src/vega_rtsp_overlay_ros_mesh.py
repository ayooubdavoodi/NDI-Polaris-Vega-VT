...
class VegaOverlayNode(Node):
    def __init__(self,
        ...
        self.pub_tracking = self.create_publisher(
            Float64MultiArray, "/KUL/marker/marker_tracking", QoSProfile(depth=10)
        )
    ...
    def _overlay_mesh(self, frame: np.ndarray, R_tool: np.ndarray, t_tool: np.ndarray,
                      color=(0, 0, 255), alpha=0.35) -> np.ndarray:
        """
        ...
        blended = self._draw_axes(blended, R_tool, t_tool)
        return blended
...
def main():
    ...
    STL_PATH = "/home/ayoob/Polaris_Vega_VT/marker_definition/Kia_phantom_marker.stl"
